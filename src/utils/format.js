...
export function formatDate(date, options = {}) {
  ...
  if (options.short) {
    ...
    if (locale === 'fr' || locale === 'es') return `${day}/${month}`
  ...
  if (locale === 'es') return `${day}/${month}/${year}` // ES: dd/mm/aaaa
  // EN — US format (mm/dd/yyyy). UK będzie rozróżniony w Phase 4 jako 'en-GB'.
...
export function formatNumber(value, options = {}) {
  ...
  const locale = getLocale()
  const formatted = (locale === 'pl' || locale === 'de' || locale === 'fr' || locale === 'es')
    ? fixed.replace('.', ',')
    : fixed
...
export function formatCurrency(amount, currency = null) {
  ...
  const cur = currency ?? (locale === 'pl' ? 'PLN' : (locale === 'de' || locale === 'fr' || locale === 'es') ? 'EUR' : 'USD')

  const fixed = amount.toFixed(2)
  const formatted = (locale === 'pl' || locale === 'de' || locale === 'fr' || locale === 'es') ? fixed.replace('.', ',') : fixed

  // Symbol position per currency
  switch (cur) {
    case 'PLN': return `${formatted} zł`
    case 'EUR': return (locale === 'de' || locale === 'fr' || locale === 'es') ? `${formatted} €` : `€${formatted}`
