    ...
    <div style={{
      ...
      <div className="card" style={{ margin: '12px 16px 0', padding: 14 }}>
        ...
        <div style={{
          ...
          {`DIAG push [BUILD=v51]: native=${String(window.Capacitor?.isNativePlatform?.() ?? 'n/a')} · avail=${String(window.Capacitor?.isPluginAvailable?.('PushNotifications') ?? 'n/a')} · webPerm=${typeof Notification !== 'undefined' ? Notification.permission : 'n/a'} · token=${fcmToken ? fcmToken.slice(0, 10) + '…' : 'brak'}`}
          {pushDebug ? ` · ${pushDebug}` : ''}
          {pushDiag ? ` || ${pushDiag}` : ''}
        </div>
      </div>
