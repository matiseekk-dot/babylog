...
export default function TempTab({ babyId }) {
      ...
      {last && (
        <div className="card" style={{padding:'16px 14px',margin:'8px 16px 0'}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{fontSize:40,fontWeight:700,color:'var(--text)'}} className={getTempClass(last.temp)}>{last.temp.toFixed(1)}°C</div>
            <div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Nowy pomiar temperatury">
        ...
        <div className="form-group">
          ...
          <select className="form-select" value={form.method} onChange={e=>setForm(f=>({...f,method:e.target.value}))}>
            <option>Odbytniczo</option>
            <option>Pod pachą</option>
            <option>W uchu</option>
            <option>Na czole</option>
          </select>
        </div>
        ...
        <div className="form-group">
          ...
          <input className="form-input" type="text" placeholder="np. po Paracetamolu" value={form.note} onChange={e=>setForm(f=>({...f,note:e.target.value}))} />
        </div>
