...
const TRANSLATIONS = {
  pl: {
    ...
    'onb.slide3.body':    'Pełna historia obserwacji i progi referencyjne z literatury medycznej (AAP, Mayo Clinic) — wszystko w jednym miejscu.',
    ...
    'disclaimer.s2.body':          'Aplikacja nie jest wyrobem medycznym. Nie stawia diagnozy ani prognozy. Nie zastępuje lekarza. Nie wylicza dawek leków. Komunikaty w aplikacji (np. progi temperatury, informacje o odstępach między dawkami) to **informacje referencyjne** z publicznej literatury medycznej (American Academy of Pediatrics, Mayo Clinic) i ulotek leków (SmPC), a nie porada medyczna dla Twojego konkretnego dziecka. Zawsze konsultuj się z pediatrą.',
    ...
    'crisis.watch.advice':    'Wytyczne AAP wskazują, że przy tej temperaturze warto monitorować dziecko. Pomocne jest mierzenie temperatury co 2 godziny. Jeśli stan się pogarsza, AAP zaleca konsultację pediatryczną.',
    'crisis.call.title':      'Próg konsultacji pediatrycznej',
    'crisis.call.action':     'AAP zaleca konsultację pediatryczną',
    'crisis.call.advice':     'Wytyczne AAP zalecają, aby w takiej sytuacji konsultować się z pediatrą. Możesz skorzystać z numeru do przychodni, teleporady albo nocnej i świątecznej pomocy lekarskiej. Decyzję o trybie konsultacji podejmujesz Ty.',
    ...
    // Crisis reasons
    'crisis.reason.very_high':       'Temperatura {temp}°C — wartość powyżej progu klasyfikowanego przez AAP jako wymagająca pilnej oceny medycznej.',
    'crisis.reason.newborn_emergency':'Temperatura {temp}°C u niemowlęcia poniżej 3. mies. — według AAP (2021) jest to próg wymagający pilnej oceny lekarskiej.',
    'crisis.reason.newborn_call':    'Temperatura {temp}°C u niemowlęcia poniżej 3. mies. — według AAP wskazana konsultacja pediatryczna.',
    'crisis.reason.young_infant':    'Temperatura {temp}°C u dziecka 3-6 mies. — według AAP w tej grupie wiekowej wskazana konsultacja pediatryczna.',
    'crisis.reason.high_fever':      'Temperatura {temp}°C — według AAP wysoka gorączka, próg konsultacji pediatrycznej.',
    'crisis.reason.watch':           'Temperatura {temp}°C — według AAP próg obserwacji. Pomocne mierzenie co 1-2h.',
    'crisis.reason.prolonged':       'Gorączka utrzymuje się > 72h — według AAP/Mayo Clinic to próg, przy którym wskazana jest konsultacja pediatryczna.',
    // Rules engine
    // === REGUŁY (informacja referencyjna) ===
    // Refactor v2.7.5: zmiana z "decision support" na "informację referencyjną
    // z literatury medycznej". Każda reguła cytuje źródło (AAP, Mayo Clinic,
    // PSO/GIS itd.). User dostaje tabelkę edukacyjną, NIE rekomendację apki.
    // Eliminuje ryzyko klasyfikacji jako wyrób medyczny pod MDR (MDCG 2019-11).
    'rule.temp_infant.title':      'Próg ostrzegawczy — niemowlę',
    'rule.temp_infant.msg':        'Temperatura {temp}°C u dziecka poniżej 3. miesiąca życia ({months} mies.) jest progiem ostrzegawczym według American Academy of Pediatrics (AAP, 2021). AAP zaleca, aby w takiej sytuacji rodzice niezwłocznie konsultowali się z pediatrą lub udali się na izbę przyjęć.',
    'rule.temp_young_infant.title':'Próg ostrzegawczy — wiek 3–6 miesięcy',
    'rule.temp_young_infant.msg': 'Temperatura {temp}°C u dziecka 3–6 miesięcy ({months} mies.). Wytyczne AAP/Mayo Clinic wskazują, że u niemowląt w tym wieku gorączka wymaga oceny pediatrycznej nawet przy niższych wartościach.',
    'rule.temp_extreme.title':     'Próg krytyczny — temperatura',
    'rule.temp_extreme.msg':       'Temperatura {temp}°C. Wartość powyżej 40,5°C jest klasyfikowana przez AAP jako stan wymagający pilnej oceny medycznej, niezależnie od wieku dziecka.',
    'rule.temp_alert.title':       'Wysoka temperatura',
    'rule.temp_alert.msg':         'Ostatni pomiar: {temp}°C. Według literatury medycznej (AAP) wartości w zakresie 38,5–39°C u dzieci powyżej 6. miesiąca to wysoka gorączka, przy której zwykle rozważa się postępowanie zgodnie z zaleceniami pediatry.',
    'rule.temp_critical.title':    'Wysoka temperatura — próg konsultacji',
    'rule.temp_critical.msg':      'Temperatura {temp}°C u dziecka powyżej 3. miesiąca jest progiem, przy którym wytyczne AAP zalecają konsultację z pediatrą. Jeśli dziecko źle wygląda, jest apatyczne lub ma inne niepokojące objawy, AAP/Mayo Clinic wskazuje izbę przyjęć.',
    ...
    'rule.med_not_working.msg':    'Temperatura nie spadła po podaniu leku {med} ({hours}h temu). Według ulotek leków przeciwgorączkowych, brak reakcji na lek może być wskazaniem do konsultacji z lekarzem. Nie podawaj kolejnej dawki poza zaleceniami z ulotki.',
    ...
    'rule.sleep_deficit.msg':      'Dziś zapisałeś/-aś {h}h {m}m snu. Normy snu według literatury (AAP): {min}–{max}h dla tego wieku. Wskaźnik obserwacyjny.',
    'rule.combined.title':         'Kombinacja niepokojących wskaźników',
    'rule.combined.msg':           'Pomiary z dziś: temperatura {temp}°C + krótki sen + niewiele karmień. Według literatury (AAP), kombinacja takich wskaźników jest progiem zalecającym pilną konsultację medyczną.',
